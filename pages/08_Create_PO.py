...
if 'analytics_category_df' not in st.session_state:
    load_and_cache_analytics_data(fetcher, None, None, category_table_id)
...
# --- FIX: Initialize Header fields only ONCE per session ---
...
if 'line_item_price_usd' not in st.session_state:
    st.session_state.line_item_price_usd = 0.0
...
def update_line_item_details():
    ...
    st.session_state.line_item_price_usd = float(cost_details.get('usd_cost', 0.0))
...
def add_item_to_draft():
    ...
    new_item = {
        ...
        "per pcs price usd": st.session_state.line_item_price_usd,
        "USD Amt": float(st.session_state.line_item_qty) * float(st.session_state.line_item_price_usd),
...
with st.container(border=True):
    ...
    with col1:
        # FIX: Use st.session_state for both reading and writing the value
...
with st.container(border=True):
    ...
    with calc_col2:
        st.number_input("Price/pcs (USD)", min_value=0.0, format="%.4f", key="line_item_price_usd")
    total_usd = float(st.session_state.line_item_qty) * float(st.session_state.line_item_price_usd)
    with calc_col3:
        st.metric("Total USD Amt", f"${total_usd:,.2f}")
    with calc_col4:
...
# ... (PO Draft display section remains the same) ...
st.divider()
...
if st.button("Create Purchase Order in Baserow", type="primary", disabled=not st.session_state.po_draft_items):
    # --- FIX: Read final values directly from session state using their keys ---
    ...
    if not final_po_number or not final_vendor_name:
        ...
        with st.spinner("Creating Purchase Order..."):
            ...
            if success_count == total_items:
                ...
                st.session_state.po_draft_items = []
                # Reset the header initialized flag to get a new PO number next time
