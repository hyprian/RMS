...
from utils.file_utils import (
    get_uploaded_sales_files,
    clear_uploaded_data_folders,
    save_session_dataframe,     # New imports
    load_session_dataframe,
    delete_session_dataframe,
    clear_all_session_dataframes
)
...
# Define filenames for session data
REPLENISHMENT_DF_FILENAME = "session_replenishment_df"
UNMAPPED_DF_FILENAME = "session_unmapped_df"
DATA_PROCESSED_FLAG_FILENAME = "session_data_processed_flag" # Using a file as a flag

# --- Function to load data from files into session_state ---
def load_persisted_data_into_session():
    if st.session_state.get('replenishment_df') is None: # Only load if not already in session (e.g. from widget interaction)
        df = load_session_dataframe(REPLENISHMENT_DF_FILENAME, project_root)
        if df is not None:
            st.session_state.replenishment_df = df
            logger.info("Loaded replenishment_df from persisted file into session state.")
        else: # If file doesn't exist, ensure session state is None
    ...
    if st.session_state.get('unmapped_df') is None:
        ...
        if df is not None:
            st.session_state.unmapped_df = df
            logger.info("Loaded unmapped_df from persisted file into session state.")
        else:
            st.session_state.unmapped_df = None
    
    if st.session_state.get('data_processed_successfully') is False: # Default is False
        # Check if the flag file exists (even if empty)
        ...
        if os.path.exists(flag_file_path):
            ...
            logger.info("Loaded data_processed_successfully flag from persisted file into session state.")
        else:
...
# --- Load persisted data on first script run or after a hard refresh ---
# This needs to be called before any widgets that might trigger a rerun
# and before the main display logic.
# We can use a flag to ensure it only runs once per "true" page load/refresh.
...
# ... (rest of config and get_data_tools remains the same) ...
...
st.sidebar.checkbox("Force refresh Baserow cache", key="force_refresh_baserow_cache",
                    help="If checked, SKU/Combo mappings will be fetched fresh from Baserow. Re-click 'Process Data'.")
with col3:
    if st.button("🔄 Reset All Data & Uploads", key="reset_all", type="primary", use_container_width=True):
        ...
        # Clear persisted session DataFrames
        ...
        st.success("All uploaded data, generated tables, and persisted session data have been reset.")
        st.rerun()
...
for platform_conf in platforms_config:
    ...
    for acc_conf in platform_conf['accounts']:
        ...
        if file_30day_path and file_60day_path:
            files_to_process_summary.append(f"- {platform_conf['name']} ({acc_conf['name']}): 30-day ({os.path.basename(file_30day_path)}), 60-day ({os.path.basename(file_60day_path)})")
            at_least_one_file_pair = True
        elif file_30day_path or file_60day_path:
             missing = "60-day" if file_30day_path else "30-day"; present_file = os.path.basename(file_30day_path or file_60day_path)
             files_to_process_summary.append(f"- {platform_conf['name']} ({acc_conf['name']}): Only one report found ({present_file}). Missing {missing} report.")
...
if process_button_clicked and process_button_disabled:
    st.error("At least one platform account needs both 30-day and 60-day sales reports to proceed.")

def read_sales_data(file_path, sku_col_name_config):
    # ... (read_sales_data function remains the same) ...
    _, file_extension = os.path.splitext(file_path)
    df = None; dtype_spec = {sku_col_name_config: str} if sku_col_name_config else None
    try:
...
if process_button_clicked and not process_button_disabled:
    with st.spinner("Processing data... This may take a moment."):
        try:
            # ... (Sales data processing logic - all_sales_data_30day, etc. - remains the same) ...
            # ... (Inventory fetching logic remains the same) ...
            # ... (Constructing final_data list with calculations remains the same) ...
            all_sales_data_30day = {}; all_sales_data_60day = {}; processed_mskus = set(); unmapped_skus_collection = []
            current_date_for_code = datetime.now(); fixed_generated_code = f"{current_date_for_code.strftime('%m-%d')}-001"
            ...
            for platform_conf in platforms_config:
                ...
                for acc_conf in platform_conf['accounts']:
                    ...
                    if qty_cols_list_config_acc and isinstance(qty_cols_list_config_acc, list): actual_qty_cols = qty_cols_list_config_acc
                    else:
                        qty_col_config_acc = acc_conf.get('quantity_column')
                        if qty_col_config_acc and isinstance(qty_col_config_acc, str): actual_qty_cols = [qty_col_config_acc]
                        else: # Fallback logic
                            default_qty_col = APP_CONFIG.get('default_sales_report_columns',{}).get('quantity_column','Quantity')
                            if default_qty_col: actual_qty_cols = [default_qty_col]
                    if not actual_qty_cols: st.error(f"Qty col(s) not configured for {p_name}-{a_name}."); continue
                    key_30day = f"{p_slug}_{a_slug}_30day"; file_30day_path = st.session_state.get('uploaded_file_paths', {}).get(key_30day) or get_uploaded_sales_files(p_slug, a_slug, "30day", APP_CONFIG)
                    key_60day = f"{p_slug}_{a_slug}_60day"; file_60day_path = st.session_state.get('uploaded_file_paths', {}).get(key_60day) or get_uploaded_sales_files(p_slug, a_slug, "60day", APP_CONFIG)
                    if not file_30day_path or not file_60day_path: continue
                    try: # 30-day
                        sales_df_30 = read_sales_data(file_30day_path, sku_col_name)
                        if sales_df_30.empty: continue; # ... (rest of 30-day processing)
                        ...
                        for _, row in sales_df_30.iterrows():
                            platform_sku_original = row[sku_col_name]; quantity = row['calculated_quantity']
                            ...
                            if msku_result:
                                ...
                                for msku_item in mskus_to_update:
                                    if msku_item: all_sales_data_30day[msku_item] = all_sales_data_30day.get(msku_item, 0) + quantity; processed_mskus.add(msku_item)
                            else:
                    ...
                    except Exception as e: st.error(f"Error processing 30-day for {p_name}-{a_name}: {e}")
                    try: # 60-day
                        sales_df_60 = read_sales_data(file_60day_path, sku_col_name)
                        if sales_df_60.empty: continue; # ... (rest of 60-day processing)
                        ...
                        for _, row in sales_df_60.iterrows():
                            ...
                            if msku_result:
                                mskus_to_update = [msku_result] if isinstance(msku_result, str) else msku_result
                                for msku_item in mskus_to_update:
                                     if msku_item: all_sales_data_60day[msku_item] = all_sales_data_60day.get(msku_item, 0) + quantity; processed_mskus.add(msku_item)
                            else:
                    ...
                    except Exception as e: st.error(f"Error processing 60-day for {p_name}-{a_name}: {e}")
            ...
            if final_data:
                ...
                with open(flag_file_path, 'w') as f: f.write('processed') # Content doesn't matter, just existence
            else:
                st.session_state.replenishment_df = None
                delete_session_dataframe(REPLENISHMENT_DF_FILENAME, project_root) # Delete if no data
                st.session_state.data_processed_successfully = True # Still processed
                flag_file_path = os.path.join(project_root, ".session_data", f"{DATA_PROCESSED_FLAG_FILENAME}.flag")
                if os.path.exists(flag_file_path): os.remove(flag_file_path)
            ...
            if unmapped_skus_collection:
                ...
                st.session_state.unmapped_df = None
                delete_session_dataframe(UNMAPPED_DF_FILENAME, project_root) # Delete if no data
            ...
            st.session_state.unmapped_df = None
            st.session_state.data_processed_successfully = False
            # Optionally delete persisted files on error too, or leave them for debugging
            delete_session_dataframe(REPLENISHMENT_DF_FILENAME, project_root)
            delete_session_dataframe(UNMAPPED_DF_FILENAME, project_root)
            flag_file_path = os.path.join(project_root, ".session_data", f"{DATA_PROCESSED_FLAG_FILENAME}.flag")
            if os.path.exists(flag_file_path): os.remove(flag_file_path)
...
# --- Display Logic (uses session state, which is now populated from files if needed) ---
if st.session_state.get('replenishment_df') is not None:
    ...
    edited_df = st.data_editor(
        st.session_state.replenishment_df.copy(), # Pass a copy to avoid direct mutation issues with editor state
        column_config=column_config,
        num_rows="dynamic",
        use_container_width=True,
        height=600,
        key="replenishment_editor"
    )
    # Check if the edited_df is different from what's in session_state (the version before this edit cycle)
    # This comparison needs to be robust. For DataFrames, .equals() is good.
    if not edited_df.equals(st.session_state.replenishment_df):
        st.session_state.replenishment_df = edited_df # Update session state
        save_session_dataframe(edited_df, REPLENISHMENT_DF_FILENAME, project_root) # Persist edits
        # st.info("Changes saved.") # Optional feedback, can be noisy
...
# Remove the flag after the page has fully rendered and used the loaded data
if 'persisted_data_loaded_this_run' in st.session_state:
    del st.session_state.persisted_data_loaded_this_run